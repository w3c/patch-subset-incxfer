...
class CborUtils {
 public:
  ...
  static cbor_item_t* EncodeString(const string& s);
  static StatusCode DecodeString(const cbor_item_t& string_item, string& out);

  static cbor_item_t* EncodeBytes(const string_view& bytes);
  static StatusCode DecodeBytes(const cbor_item_t& bytes_item, string& out);

  static set<int> MapKeys(const cbor_item_t& map);

  static StatusCode SerializeToBytes(const cbor_item_t& item,
                                     string_view buffer, size_t* bytes_written);
};
